...
const InsurerTable: React.FC<InsurerTableProps> = ({ coverageLimit }) => {
  ...
  return (
    <div className="w-full bg-white rounded-lg shadow-md mt-10">
      ...
      <div className="mt-0 text-sm text-center relative bottom-3 right-3">
...
//   const [selectedRows, setSelectedRows] = useState<string[]>([]);
//   const [totalSelectedCoverage, setTotalSelectedCoverage] = useState<number>(0);
//   const [totalPremium, setTotalPremium] = useState<number>(0); // New state for total premium
...
//   return (
//     <div className="w-full bg-white rounded-lg shadow-md mt-10"> {/* Added margin-top to shift the table down */}
//       {/* Insurer Table */}
//       <div className="max-h-80 overflow-y-auto mt-4"> {/* Decreased the margin-top here */}
//         <table className="w-full text-left border-collapse mb-4 text-sm">
//           <thead className="sticky top-0 bg-white z-10">
//             <tr className="border-b">
//               <th className="p-3 font-semibold text-gray-700">Insurer Id</th>
...
//           <tbody>
//             {insurerData.map((insurer) => {
//               const isSelected = selectedRows.includes(insurer.insurerId);
//               const wouldExceedLimit = totalSelectedCoverage + insurer.coverage > coverageLimit;

//               return (
//                 <tr
//                   key={insurer.insurerId}
//                   className={`border-b cursor-pointer ${isSelected && wouldExceedLimit
//                     ? 'bg-red-100 text-red-500'  // Change color only when total coverage exceeds limit
//                     : isSelected
//                     ? 'bg-blue-100'
//                     : 'bg-white'
//                     }`}
...
//       </div>
//       {/* Total selected policies and coverage */}
//       <div className="mt-2 text-sm text-center relative">
//         <p className="font-semibold">Total Premium: <span className="font-bold text-blue-600">${totalPremium.toLocaleString()}</span></p>
//       </div>
//       <div className="text-sm flex flex-col">
//         <p className="font-semibold">Total Selected Policies: {selectedRows.length}</p>
//         <p className={`font-semibold ${totalSelectedCoverage > coverageLimit ? 'text-red-500' : ''}`}>
//           Total Selected Coverage: {totalSelectedCoverage.toLocaleString()} / {coverageLimit.toLocaleString()}
//         </p> {/* Change text color if it exceeds */}
//       </div>
...
//             {insurerData.map((insurer) => {
//               const isSelected = selectedRows.includes(insurer.insurerId);
//               const exceedsLimit = totalSelectedCoverage + insurer.coverage > coverageLimit;

//               return (
//                 <tr
//                   key={insurer.insurerId}
//                   className={`border-b cursor-pointer ${isSelected
//                     ? exceedsLimit
//                       ? 'bg-red-100 text-red-500'  // Only change color when it exceeds
//                       : 'bg-blue-100'
//                     : 'bg-white'
//                     }`}
//                   onClick={() => handleRowClick(insurer.insurerId, insurer.coverage, insurer.premium)}
...
//       <div className="text-sm flex flex-col">
//         <p className="font-semibold">Total Selected Policies: {selectedRows.length}</p>
//         <p className="font-semibold">Total Selected Coverage: {totalSelectedCoverage.toLocaleString()} / {coverageLimit.toLocaleString()}</p>
//       </div>
...
//       {/* Total selected policies and coverage */}
//       {/* Total premium and selected policies */}
//       <div className="mt-2  text-sm text-center relative bottom-3 right-3">
//         <p className="font-semibold">Premium: <span className="font-bold text-blue-600">{totalPremium.toLocaleString()}</span></p>
//       </div>
//       <div className="text-sm flex flex-col">
//         <p className="font-semibold">Total Selected Policies: {selectedRows.length}</p>
